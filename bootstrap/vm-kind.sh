function bootstrap_precheck() {
    ...
    hcloud ssh-key create --name "${CLUSTER_NAME}-jump" --public-key-from-file ./ssh-jump.pub --label type="${CLUSTER_NAME}-jump"
    # TODO: Check for existence
    hcloud server create --name "${CLUSTER_NAME}-jump" --type cx32 --image ubuntu-24.04 --ssh-key "${CLUSTER_NAME}-jump" --label type="${CLUSTER_NAME}-jump"
